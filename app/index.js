...
class MyBetweenTime extends Component {
    ...
    constructor(props) {
        ...
        this.doUpdateDataSourceWithoutAPI = this.doUpdateDataSourceWithoutAPI.bind(this);
    ...
    doAddDataToList(cityTime) {
        ...
        DAOAndroidStorage.putObjectWithKey(Constant.modelMediator.REPOSITORY, this.state.dataSource)
            .catch(error => {
    ...
    async doUpdateDataSourceWithAPI(dataSource) {
        ...
        for (let i = 0; i < dataSource.length; i++) {
            ...
            if(!this.state.networkStatus){
                ...
                await ObtainTimeZoneToAPI.createRequestByURL(city.urlOnline).then(result => {
                    let newDate = new Date(result.year,
                        result.month,
                        result.day,
                        result.hours,
                        result.minutes,
                        result.seconds,
                        result.millis);
                    console.debug(LOG_TAG, 'Time update with API: ', newDate.toTimeString());
                    city.updateTimeWithData(newDate);
                    dataSource[i] = city;
                    console.debug(LOG_TAG, 'Time inside city updated: ', city.toString());
                }).catch(error => {
                    this.doPrintError(error.message);
                });
            }
        ...
        let messagge = this.state.networkStatus === true ? 'Local times updated' : 'Connection not enabled';
        this.setState({
            dataSource: dataSource,
            loading: false,
            toastVisible: true,
    ...
    doUpdateDataSourceWithoutAPI() {
        ...
        if (dataSource && dataSource.length !== 0 && this.state.networkStatus) {
            ...
            this.state.dataSource.forEach(city => {
                ...
                let newTime = Moment(timeCity).add(1, 'minutes').toDate();
    ...
    async componentDidMount(): void {
        ...
        await DAOAndroidStorage.getObjectWithKey(Constant.modelMediator.INIT).then(init => {
            console.log(LOG_TAG, 'Component did mont value init: ', init);
            if (init === false) {
                DAOAndroidStorage.putObjectWithKey(Constant.modelMediator.INIT, true).then(() => {
                    this.setState({
                        appInitialized: true,
                    });
                }).catch(error => {
                ...
                DAOAndroidStorage.getObjectWithKey(Constant.modelMediator.REPOSITORY).then(dataSource => {
    ...
    render() {
        return (
            <PaperProvider theme={LITE_THEME}>
                <StatusBar backgroundColor={LITE_THEME.colors.primary} barStyle="light-content"/>
                ...
                <SafeAreaView style={GlobalStyle.droidSafeAreaDown}>
                        <Appbar.Header style={GlobalStyle.appBar}>
                            ...
                            />
                            <Appbar.Action icon="refresh" disabled={!this.state.networkStatus} onPress={() => {
                                this.doUpdateDataSourceWithAPI(this.state.dataSource)
                                    .catch(error => {this.doPrintError(error.message)});
                            }}/>
                        ...
                        />
                        <Snackbar style={GlobalStyle.snackbar}
                                  theme={LITE_THEME}
